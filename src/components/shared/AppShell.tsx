...
export function AppShell({ children, counterKind, lowStockCount = 0, role }: AppShellProps) {
  ...
  return (
    <div className="flex flex-col h-dvh overflow-hidden bg-white print:h-auto print:overflow-visible" style={{ fontFamily: "var(--font-outfit, var(--font-sans))" }}>
      ...
      <div className="flex flex-1 overflow-hidden print:block print:overflow-visible">
        ...
        {/* Desktop sidebar */}
        <aside className="hidden md:flex flex-col w-56 border-r border-black/5 p-3 gap-0.5 flex-shrink-0 overflow-y-auto print:hidden">
        ...
        {/* Mobile icon rail */}
        <aside className="flex md:hidden flex-col w-14 py-3 px-1.5 shrink-0 border-r border-black/5 overflow-y-auto print:hidden">
          {navItems.map((item) => {
            const isActive = pathname === item.href || pathname.startsWith(item.href + "/");
            return (
              <Link key={item.label} href={item.href}
        ...
        {/* Main content slot */}
        <main className={`${isFullBleed ? "flex flex-1 flex-col overflow-hidden" : "flex-1 overflow-y-auto p-4 md:p-5 space-y-4 md:space-y-5"} print:h-auto print:flex-none print:overflow-visible`}>
          {children}
        </main>
      </div>
    </div>
