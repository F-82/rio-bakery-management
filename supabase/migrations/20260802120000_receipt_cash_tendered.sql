-- Client request (Manuja): "add the cash given balance to the bill" — put
-- the cash the customer handed over and the change owed back on the printed
-- customer receipt.
--
-- payload gains an optional cash_given (numeric >= 0). It is purely a
-- receipt annotation: it never touches orders.subtotal/discount/total or any
-- revenue figure (Invariant 3 — no adjustment to what was actually charged),
-- and it is only surfaced for cash payments. Change owed is computed here,
-- server-side, as greatest(cash_given - total, 0) so an under-tendered
-- amount never prints a negative "change". Because print_jobs.payload is
-- stored, both the first print and any later reprint from order history show
-- these lines.
--
-- Full replace (forward-only migrations can't edit the earlier create_order
-- in place) — every step is unchanged from 20260729153625_cap_change_to_points
-- except the new v_cash_given parse/validation and the cash-tender object
-- folded into the customer_receipt payload in step 8.
...
begin
  ...
  v_cash_given  := coalesce(nullif(payload ->> 'cash_given', '')::numeric, 0);
  ...
  if v_cash_given < 0 then
    raise exception 'cash_given cannot be negative';
  end if;
  ...
  --    prep, carrying only prep lines and no prices. Cash-tender lines
  --    (cash_given/change_due) are added to the receipt only for cash
  --    payments where an amount was actually entered — never on card, and
  --    never as a revenue field (Invariant 3). change_due is floored at 0 so
  --    an under-tendered amount can't print a negative change.
  ...
  select v_business_id, v_order_id, 'customer_receipt',
    ...
    || case
         when v_payment = 'cash' and v_cash_given > 0 then
           jsonb_build_object(
             'cash_given', v_cash_given,
             'change_due', greatest(v_cash_given - v_total, 0),
             'change_to_points', v_change_to_points_lkr
           )
         else '{}'::jsonb
       end;
