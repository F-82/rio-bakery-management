-- Client request: when cash change owed is small (their example: ~LKR 7)
-- and staff don't have coins, offer to credit that amount to the
-- customer's loyalty balance instead of shorthanding them. Full replace
-- (forward-only migrations can't edit 20260728064013_loyalty.sql in
-- place) — every step is unchanged from that file except 6b, which now
-- also folds in a client-requested "bonus" amount converted to points at
-- the same earn rate as ordinary spend. Placeholder mechanism: reuses
-- loyalty.earn_points_per_lkr (STEPS.md client blocker #2 — the owner
-- hasn't signed off on rates yet), flagged for confirmation like every
-- other loyalty default in this codebase.
--
-- payload gains an optional change_to_points_lkr (numeric >= 0). Requires
-- a customer_id — with no customer there's no account to credit, so the
-- amount is silently dropped rather than erroring (mirrors redeem_points
-- needing a customer). Never trusted as a discount or total adjustment:
-- it only feeds points_earned, orders.total is unaffected (Invariant 3).
